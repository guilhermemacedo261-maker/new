...
function AdminGamesInner() {
  ...
  async function reload() {
    if (!weekId) return;
    const res = await fetch(`/api/admin/games?weekId=${weekId}`).then((r) => r.json());
    setGames(res.games ?? []);
    setLoading(false);
  }
  ...
  return (
    <div className="p-4 md:p-8">
      <h1 className="font-display text-2xl mb-1">Jogos da semana</h1>
      <p className="text-xs text-buteco-white/40 mb-6">
        Corrigir um jogo aqui trava ele contra a busca automática (a API da NFL não sobrescreve mais esse jogo
        específico) e já recalcula ranking, histórico e ao vivo na hora.
      </p>
      <div className="space-y-3">
        {games.map((game) => (
          <div key={game.id} className="bg-buteco-charcoal rounded-xl p-4">
            {game.manually_corrected && (
